...
salida=out.Data';
aux1=[];
for i=1:9:length(salida)
    if i+8 < 207
        aux1=[aux1;salida(i:i+8)];
    else
        aux1 =[aux1;[salida(i:end),salida(206)]];
    end
end
aux1 = dec2bin(aux1);
aux2=[];
for i=1:23:length(salida)
        aux2=[aux2,aux1(i:i+22)];
end
paridad1=aux2(:,end);
texto = char(bin2dec(aux2(1:23,1:8)));
%obtengo la paridad del mensaje recibido 
b=dec2bin(texto, 8);
[f,c]=size(b);
binary2=zeros(f,c+1);
for i=1:f
