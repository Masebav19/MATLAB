...
Fs=linspace(0,100,200);
Vrms = zeros(size(Fs));
for i=1:length(Fs)
t=linspace(0,20,20000000);
...
ylabel('Voltaje [V]');
legend('Respuesta en el dominio de la frecuencia');
...
P = zeros(size(c0));
for i=1:length(c0)
    GP=(-alpha^2*R*s^2*(m1*s^2+k1+c1*s)^2)/(2*((m1*s^2+k1+c1*s)*((k2+c2*s+m2*s^2 )*(1+R*c0(i)*s)+alpha^2*R*s)+...
    ((k2+c2*s)*(1+R*c0(i)*s)+alpha^2*R*s)*m2*s^2)^2);
t=linspace(0,20,200000);
u=Am*sin(2*pi*Fs*t);
y=lsim(GP,u,t);
P(i)=sqrt(trapz(t,y.^2)/t(end));
...
figure
plot(c0,1000.*P)
title('Respuesta de potencia ante variación de parámetros');
xlabel('Capacitancia del piezoeléctrico [F]');
ylabel('Potencia [mW]');
...
% se multiplca por la amplitud al cuadrado porque la función de traferencia
% de la potencia de salida es P/F^2 faltando una amplitud de Am^2 que es la
% amplitud de la armónica 
...
m1=270;
m2=54;
c1=linspace(0.1*42.98,1e2*42.98,100);
c2=98.26e3;
c0=1.89e-8;
k1=22.89e6;
k2=192.60e6;
...
figure
plot(c1./42.98,1000.*P,'b')
title('Respuesta de la potencia')
xlabel('coeficiente de amortiguamiento [x42.98 N*s/m]')
ylabel('Potencia [mW]')
...
m1=270;
m2=54;
c1=linspace(0.1*42.98,50*42.98,100);
c2=98.26e3;
c0=1.89e-8;
k1=22.89e6;
k2=192.60e6;
...
%% Varacion de parametros C0 y alpha

m1=270;
m2=54;
c1=42.98;
c2=98.26e3;
c0=1.89e-8;% valores de la capacitancia
k1=22.89e6;
k2=192.60e6;
alpha=1.52e-3;%valores del factor de fuerza
R=30455.3;

% contantes del piezo 
K=3800; %contante de permitividad del piezo
epsilon0=8.854e-12;%permitividad en el vacìo
e33=(alpha/c0)*K*epsilon0;% contante de esfuerzo del piezo 
% 
Am=140e3;%amplitud de la entrada armónica
%area del piezo 
A=linspace(1e-3*(14e-2*70e-2),14e-2*70e-2,10);
L=linspace(0.1e-3,0.51e-3,10);
s=tf('s');
data=zeros(length(A),6);
for i=1:length(A)
    for j=1:length(L)
        c0=K*epsilon0*A(i)/L(j);
        alpha=e33*A(i)/L(j);
            GP=(-alpha^2*R*s^2*(m1*s^2+k1+c1*s)^2)/(2*((m1*s^2+k1+c1*s)*((k2+c2*s+m2*s^2 )*(1+R*c0*s)+alpha^2*R*s)+...
            ((k2+c2*s)*(1+R*c0*s)+alpha^2*R*s)*m2*s^2)^2);
        [V,n]=getPeakGain(GP);
        data(length(A)*(j-1)+i,1)=0.02407*Am*V;
        data(length(A)*(j-1)+i,2)=n/(2*pi);
        data(length(A)*(j-1)+i,3)=c0;
        data(length(A)*(j-1)+i,4)=alpha;
        data(length(A)*(j-1)+i,5)=A(i);
        data(length(A)*(j-1)+i,6)=L(j);
    end
end
% figure
% plot(data(:,2),data(:,1));
data=array2table(data,'VariableName',{'Potencia','freq','Capacitancia','Factor de fuerza','Area','Espesor'});
writetable(data,'datos_tesis.txt');

%% Potencia vs amplitud de entrada
clear
clc
m1=270;
m2=54;
...
Am=linspace(135e3,200e3,100);%amplitud de la entrada armónica
...
for i=1:length(Am)
t=linspace(0,20,200000);
u=Am(i)*sin(2*pi*Fs*t);
y=lsim(GP,u,t);
P(i)=sqrt(trapz(t,y.^2)/t(end));
end
figure
plot(Am./1000,1000.*P)
title('Gráfica de respuesta de la potencia en función de la amplitud armónica');
xlabel('Fuerza [KN]');
ylabel('Potencia [mW]');
